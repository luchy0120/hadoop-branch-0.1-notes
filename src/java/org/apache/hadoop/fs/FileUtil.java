...
public class FileUtil {
    ...
    // 如果src是一个文件，（dst也是一个文件）那么将文件copy到dst文件里，如果src是一个directory，
    // 将src底下的directories和文件们都拷贝到dst文件夹下(包括checksum文件)
    /**
    ...
    // dst必须是一个目录，如果src是一个文件，将src文件拷贝到dst下
    // 如果src是文件夹，拷贝底下的文件夹和文件到dst下（不包括checksum文件）
    /**
