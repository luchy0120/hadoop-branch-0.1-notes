...
public class SequenceFile {
  ...
  public static class Writer {
    ...
    // version + keyclassname + valueclassname + isdefalte + sync
    // 写文件头部
    ...
    // 先写key和value的总byte长度，再写key的长度，再写key，再写value
    ...
    // 若当前的写的位置距离上次的sync位置超过了一个interval，那么在当前
    // 位置写一个16byte的sync值
    /** Append a key/value pair. */
  ...
  /** Read key/value pairs from a sequence-format file. */
