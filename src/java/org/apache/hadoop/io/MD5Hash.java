...
public class MD5Hash implements WritableComparable {
  ...
  public boolean equals(Object o) {
   ...
   // 根据16个byte密码算hash，每4位拼接成一个int后，对4个int取xor
  ...
  public String toString() {
    ...
    for (int i = 0; i < MD5_LEN; i++) {
      int b = digest[i];
      // 把一个byte拆分成两个nibble，HEX_DIGITS是nibble到char的mapping
  ...
  public void setDigest(String hex) {
    ...
    // 将每两位的16进制字符，变成nibble后，将他们拼成8位的byte
    for (int i = 0; i < MD5_LEN; i++) {
      int j = i << 1;
  ...
  //一个char变成一个int的低四位值，nibble就是一个int的最低4位
