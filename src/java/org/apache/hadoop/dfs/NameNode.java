...
// 文件 -> blocks (namespace)，存在磁盘上
// block -> machines (inodes)，每次启动namenode后构建

// 客户端 操作DistributedFileSystem，而它又会操作DFSClient，而它又使用ClientProtocol的代理client
// 来与namenode进行对话
/**********************************************************
 ...
 **********************************************************/
// 实现了两个协议，一个是被client所调用的接口协议，这里的client是指DFSClient，而user使用的是DistributedFileSystem
// 另一个是被datanode所调用的接口协议
public class NameNode implements ClientProtocol, DatanodeProtocol, FSConstants {
    ...
    // namenode启动RPCserver，这个server会获取client的Invocation，解析出method后调用
    // namenode的相应方法。 FSNamesystem管理name数据的存储
    /**
    ...
    */
    ...
    // ClientProtocol
    /////////////////////////////////////////////////////
    // 转给namesystem去获得所有的Block和它们的所在datanode位置，随后client可以读数据
    ...
    // 转给namesystem去创建一个新的文件并返回它的第一个Block
