...
class FSNamesystem implements FSConstants {
    ...
    //
    // <String,Vector> string是文件名，vector存放blocks
    TreeMap pendingCreates = new TreeMap();

    //
    // Keeps track of the blocks that are part of those pending creates
    //
    // <block> 存放所有属于某个刚新建的文件的Block，这些block只代表了文件的一部分，
    // 如果client放弃了写完整个文件，那么这些block会被丢弃
     ...
     * is stored
     */
    // dir里面保存着整个分布式文件系统的状态数据，比如文件名以及它包含的block信息
     ...
     * at random.
     */
    // 获取一个文件的所有block信息，并从blocksmap里面获取block对应的datanodes
     ...
     */
    // 新建一个文件，返回一个新的Block，holder是client名字
    ...
    public synchronized int completeFile(UTF8 src, UTF8 holder) {
        // FSDirectory中记录了Blocks或者pendingCreates里没记录这些Blocks，异常情况
        if (dir.getFile(src) != null || pendingCreates.get(src) == null) {
     ...
     */
    // 检查每个Block，的datanodes，如果为空或者数量不够，说明这个Block还没有复制到其他节点上，
    // 需要继续等待
    ...
    /**
     */
    // 将当前的datanodeMap的value复制一份出来
    public DatanodeInfo[] datanodeReport() {
        DatanodeInfo results[] = null;
        synchronized (heartbeats) {   // 不要跟heartbeats 有冲突，因为heartbeat时会修改datanodeMap
            synchronized (datanodeMap) {
