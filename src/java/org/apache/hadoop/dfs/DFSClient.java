...
class DFSClient implements FSConstants {
    ...
    class DFSInputStream extends FSInputStream {
        ...
        // 从namenode获取文件的所有Block信息（包括了每个Block的id和在哪些datanode上存在）
        /**
    ...
    class DFSOutputStream extends FSOutputStream {
        ...
        private synchronized void nextBlockOutputStream() throws IOException {
            ...
            do {
                ...
                while (! blockComplete) {
                    if (firstTime) {
                        // clientName 就是 DFSClient-23173712 ， localname就是本地hostname
                ...
                // 向namenode申请到了block信息后，开始连datanode写数据
                block = lb.getBlock();
                ...
                try {
                    ...
                    if (firstTime) {
                        // 失败了，向namenode报告我放弃了这个文件
                        namenode.abandonFileInProgress(src.toString());
                    } else {
                        // 失败了，向namenode报告我放弃了这个block
                ...
                //
                DataOutputStream out = new DataOutputStream(new BufferedOutputStream(s.getOutputStream()));
                out.write(OP_WRITE_BLOCK);    // 我要写一个block
                out.writeBoolean(false);
                block.write(out);              // block的信息，block的id
                out.writeInt(nodes.length);    // 多少个datanode
                for (int i = 0; i < nodes.length; i++) {
                    nodes[i].write(out);     // 写出每个datanode信息
                }
                out.write(CHUNKED_ENCODING);
                bytesWrittenToBlock = 0;
                blockStream = out;       // 写出的stream
                blockReplyStream = new DataInputStream(new BufferedInputStream(s.getInputStream()));    /// 写出block的应答stream
            } while (retry);
            firstTime = false;
        }
      ...
      public synchronized void write(byte b[], int off, int len)
        throws IOException {
            ...
            while (len > 0) {
              int remaining = BUFFER_SIZE - pos;      // buffer的空闲
              int toWrite = Math.min(remaining, len);    // bytes数组要写出多少
              System.arraycopy(b, off, outBuf, pos, toWrite);   // 从bytes数组写到outBuf里
              pos += toWrite;    // 写了，pos在buffer里往后走
              off += toWrite;    // 写了off在bytes里往后走
              len -= toWrite;    // 写了，len剪掉
              filePos += toWrite;   // 文件写了多少个

              if ((bytesWrittenToBlock + pos >= BLOCK_SIZE) ||
                  (pos == BUFFER_SIZE)) {      // pos走到底了
                flush();
              }
            }
        }
        ...
        private synchronized void flushData(int maxPos) throws IOException {
            ...
            if (workingPos > 0) {
                ...
                //
                backupStream.write(outBuf, 0, workingPos);     // 先写到本地backup文件中

                //
                // Track position
                //
                bytesWrittenToBlock += workingPos;      // 记录下写出去多少bytes
        ...
        private synchronized void endBlock() throws IOException {
            ...
            while (mustRecover) {
                ...
                try {
                    ...
                    while (bytesRead > 0) {
                        blockStream.writeLong((long) bytesRead);  // 先写bytes的长度
                        blockStream.write(buf, 0, bytesRead);    // 再写bytes数据
                        bytesRead = in.read(buf);     // 从本地文件里读 BUFFER_SIZE
                    }
                    internalClose();
                    mustRecover = false;
                } catch (IOException ie) {
            ...
            //
            backupFile.delete();               // 删掉
            backupFile = newBackupFile();      // 创建新的
        ...
        private synchronized void internalClose() throws IOException {
            blockStream.writeLong(0);   // 发个0，表示我不发了
            blockStream.flush();

            long complete = blockReplyStream.readLong();     // datanode回应“我知道了”
            ...
            lb.readFields(blockReplyStream);     // datanode回应block信息
            namenode.reportWrittenBlock(lb);     // 将这个block信息再转述给namenode，表示我已经写完一个block了
